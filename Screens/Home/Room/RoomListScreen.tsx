...
import { backgroundColor, darkerBackgroundColor, windowHeight, windowWidth } from "../../../Constants/cssConst";
          ...
          <>
            <View style={{ height: roomsLayout(rooms!.length) }}>
              <FlatList
                ...
                renderItem={({ item }) => {
                  return <RoomBox key={item.room.id} onPressHandler={() => goRoom(item.room.id, item.room.name)} roomName={item.room.name} />;
                }}
              />
            </View>
...
const CreateRoomBlock: React.FC<CreateRoomBlockProps> = ({ createRoomFunc }) => {
  return (
    <TouchableOpacity
    ...
    >
      <Entypo name="circle-with-plus" size={windowWidth * 0.13} color={backgroundColor} />
      <Text
        style={{
